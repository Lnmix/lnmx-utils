...
function log(msg, options, callback) {
	var out = getStream(options.path),
		respectInclude = false,
		obj;

	if (options.hasOwnProperty('include') && options.include.length) {
		respectInclude = true;
		obj = JSON.parse(msg);
	}

	if (!respectInclude || (obj.hasOwnProperty('eventId') && options.include.indexOf(obj.eventId) >= 0))
		out.write(msg + "\n");
	if (callback)
		callback();
}
