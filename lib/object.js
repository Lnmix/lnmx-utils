 ...
 *
 * @param {Object|Array|Date} obj The item to be cloned.
 * @return {Object|Array|Date} The cloned item.
...
/**
 * Merge arguments recursively, right to left
 * @param object {Object}
 * @param [...] {Object}
 * @return {Object}
 */
var mergeRecursive = function() {
	...
	var left = args.shift();
	var right = (args.length > 1) ? mergeRecursive.apply(null, args) : args.pop();
	Object.keys(right).forEach(function(key){
		if (left.hasOwnProperty(key) && 'object' === typeof left[key] && 'object' === typeof right[key]) {
			// recursively merge objects, right to left
			left[key] = mergeRecursive(left[key], right[key]);
		} else {
			// merge value (potentially overwrites), right to left
			left[key] = right[key];
		}
	});
	return left;
...
/**
 * Render an object properties as string
 * Example: 
 * renderProperties({foo: 1, bar: "test"}, '=', ' ')
 * foo=1 bar="test"
 *  
 * @param {Object} obj       Object to render
 * @param {String} operator  defaults to "="
 * @param {String} separator defaults to " " (whitespace)
 * @return {String}
 */
var renderProperties = function(obj, operator, separator) {
	var output = [];
	Object.keys(obj).forEach(function(key) {
		if (obj.hasOwnProperty(key)) output.push(key + (operator||'=') + JSON.stringify(obj[key]));
	});
