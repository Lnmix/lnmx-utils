...
function log(logLevel, messages, callback) {
	if (config.log && config.log.length) {
		...
		async.forEach(config.log, function(target, cb) {
			var toLog = false;
			if (target) {
				if (!target.hasOwnProperty('logLevel') || logLevel <= target.logLevel)
					toLog = true;
				if (target.hasOwnProperty('maxLevel') && logLevel < target.maxLevel)
					toLog = false;
				if (target.hasOwnProperty('onlyLevel') && logLevel != target.onlyLevel)
					toLog = false;
			}
			if (toLog)
