...
function log(logLevel, messages, callback) {
	if (config.log && config.log.length) {
		...
		async.forEach(config.log, function(target, cb) {
			...
			if (!target)
				toLog = false;
			else if (target.hasOwnProperty('logLevel') && logLevel > target.logLevel)
				toLog = false;
			else if (target.hasOwnProperty('maxLevel') && logLevel < target.maxLevel)
				toLog = false;
			else if (target.hasOwnProperty('onlyLevel') && logLevel != target.onlyLevel)
				toLog = false;
...
function logToTarget(target, logObj, callback) {
	...
	try {
		engine = engines[String(target.engine).toLowerCase()];
		formatter = formatters[String(target.formatter).toLowerCase()];
	}
	catch (e) {
		failure = e;
	}
